...
const Timer: React.FC<TimerProps> = () => {
  ...
  // 🔊 Audio: create and keep one Audio element
  const audioRef = useRef<HTMLAudioElement | null>(null);
  useEffect(() => {
    audioRef.current = new Audio(alarmSound);
    audioRef.current.preload = 'auto';
    audioRef.current.volume = 0.8; // tweak if needed
  }, []);
  ...
  // Start/Pause toggle
  const handleStart = () => {
    // Prime audio on first user gesture so browsers allow playback later
    if (audioRef.current) {
      try {
  ...
  useEffect(() => {
    ...
    // Play sound on both work and break completion
    playAlarm();
